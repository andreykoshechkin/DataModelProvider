...
public interface ExcelDataRepository extends JpaRepository<Deposit, Long> {
    ...
    @Query("select new org.example.datamodelprovider.data.ExcelDataProjection(dep.id, dep.email) " +
            "from Deposit dep " +
            "where dep.dtOpen = :yesterday " +
            "and dep.serviceType = :serviceType " +
            "and dep.requestType = :requestType ")
    List<ExcelDataProjection> getExcelDataProjection(@Param("serviceType") String serviceType,
    ...
    @Query("select new org.example.datamodelprovider.data.ExcelDataProjection(dep.id, dep.email) " +
            ...
            "and dep.serviceType = :serviceType " +
            "and dep.requestType = :requestType")
    Page<ExcelDataProjection> getExcelDataProjectionPage(@Param("yesterday") LocalDate yesterday,
